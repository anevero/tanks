...
class Bot : public Tank {
 public:
  Bot(std::shared_ptr<const Map> map, int init_cell_x, int init_cell_y,
 ...
 protected:
  ...
  bool IsWallBetweenObjectsX(
      int tank_x, int tank_y, int bot_x, int bot_y) const;

  bool IsWallBetweenObjectsY(
      int tank_x, int tank_y, int bot_x, int bot_y) const;
};
